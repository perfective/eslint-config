...
is overridden and errors for: `acc`, `arr`, `attr`/`attrs`, `btn`, `cb`, `conf`, `ctx`, `cur`/`curr`, `dest`, `dev`,
`dir`/`dirs`, `e`, `el`, `elem`, `envs`, `err`, `ev`/`evt`, `ext`/`exts`, `fn`/`func`, `idx`, `len`, `mod`, `msg`,
`num`, `obj`, `opts`, `pkg`, `prev`, `prod`,
`prop`/`props` (allowed in `@perfective/eslint-config-react`),
`ref`/`refs` (allowed in `@perfective/eslint-config-angular` and `@perfective/eslint-config-angular`),
`rel`, `req`, `res`, `ret`, `retval`, `sep`, `src`, `stdDev`, `str`, `tbl`, `temp`, `tit`, `tmp`,
`val`, `var`/`vars`, `ver`.

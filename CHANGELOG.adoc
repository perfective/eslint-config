...
+
* Upgraded `@typescript-eslint` to `5.12.0`:
** specified `set` and `get` methods to be ordered after regular methods.
+
* Upgraded `eslint-plugin-jest` to `26.1.1`:
...
+
* Upgraded `eslint-plugin-testing-library` to `5.0.5`:
** Renamed `testing-library/no-debug` into `testing-library/no-debugging-utils`.
+
* Upgraded `eslint-plugin-unicorn` to `41.0.0`:
...
+
* Updated `eslint-plugin-jsdoc` to `37.9.3:

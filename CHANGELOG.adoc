...
** Enabled the `link:{eslint-jsdoc-rules}#no-blank-blocks[jsdoc/no-blank-blocks]` rule as a warning.
** Enabled the `link:{eslint-jsdoc-rules}#tag-lines[jsdoc/tag-lines]` rule as a warning.
** Configured the `link:{eslint-jsdoc-rules}#sort-tags[jsdoc/sort-tags]` rule to group tags.

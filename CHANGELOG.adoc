...
** `typescriptEslintTslintConfig` to override `@typescript-eslint/tslint/config`
(removed `@perfective/eslint-config/tslint`);
** `simpleImportSortImports` to add internal scope packages groups in `simple-import-sort/imports`;
** `unicornPreventAbbreviations` to extend replacements in `unicorn/prevent-abbreviations`.
* Changes in configuration:
** Allowed to use a function or a class in `describe()` name.
** Allowed to skip description for JSDocs marked as `@private` or `@package`.
...
** Allowed leading underscores for unused parameters and private properties.
** Allowed `UPPER_CASE` for constants and static properties names.
